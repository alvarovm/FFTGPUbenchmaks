...
// Built with: dpcpp FFT_c2r_backward.cpp -DMKL_ILP64 -lmkl_sycl -lmkl_intel_ilp64 -lmkl_sequential -lmkl_core -L$HOME/fftpack -lfftpack
...
void fft_c2r_nonbatch_onemkl(double* a) {
    ...
    for (int q=0; q<nq1; ++q) {
	auto event = compute_backward(desc, &(x_usm[indx]));
	indx += (nx+2);
...
void fft_c2r_batch_onemkl(double* a) {
    sycl::queue dev_que(sycl::gpu_selector{},
			    sycl::property_list{sycl::property::queue::enable_profiling{},
				    sycl::property::queue::in_order{}});
...
int main(int argc, char** argv) {
    ...
    fft_c2r_nonbatch_onemkl(inout_nonbatch_onemkl);
    fft_c2r_batch_onemkl(inout_batch_onemkl);
